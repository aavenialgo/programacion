...
def load_ppg_from_csv(filepath):
    ...
    try:
        df = pd.read_csv(
            filepath, 
            usecols=["tiempo_relativo_s", "valor_crudo"]
            #tiempo_s	ppg_suavizada

        )
        signal = df["valor_crudo"].to_numpy()
        avg_sample_period = np.mean(np.diff(df["tiempo_relativo_s"]))
...
ARCHIVO_CSV = 'src/data/datos_ppg_prueba27_11_canal_crudo.csv'
...
if signal is not None and fs is not None:
    ...
    if heartpy_wd is not None and heartpy_m is not None:
        ...
        # HeartPy a veces no llama a show() automaticamente
        plt.show()
